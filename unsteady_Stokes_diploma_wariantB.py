...
# zdefniowanie wyrażeń i stałych wartości
...
solve(F1 == 0, _w1, [bcu_inflow, bcu_walls])
u1, p1 = _w1.split()
...
solve(F2 == 0, _w2, [bcu_inflow, bcu_walls])
u2, p2 = _w2.split()
...
solve(F3 == 0, _w3, [bcu_inflow, bcu_walls])
u3, p3 = _w3.split()
...
solve(F4 == 0, _w4, [bcu_inflow, bcu_walls])
u4, p4 = _w4.split()
...
xdmf_file0 = XDMFFile('mgr_x/solution_u.xdmf')
xdmf_file1 = XDMFFile('mgr_x/solution_p.xdmf')
            ...
            + q0 * div(w0) * dx \
                - dot(g , v0) * dx \
                    - alpha_1/k * dot(un, v0) * dx \
                        - alpha_1/k * gamma1 * f1 * dot(v0, n) * ds(1) \
                            - alpha_1/k * gamma2 * f2 * dot(v0, n) * ds(2) \
                                - alpha_1/k * gamma3 * f3 * dot(v0, n) * ds(3) \
                                    - alpha_1/k * gamma4 * f4 * dot(v0, n) * ds(4) 


for qqq in trange(num_steps):
            

    solve(F0 == 0, _w0, [bcu_inflow, bcu_walls])
    
    Q_2 = 50 * np.sin(qqq/100 * np.pi)
    Q_3 = 50 - Q_2
    Q_4 = 50 + Q_2
